...
| :--- | :--- | ---: | :--- | :--- |
| 诸葛亮联名羽扇 | 诸葛亮联名羽扇 | 50 | 件 | 保留旧核心断言，正常库存 |
| 木牛流马模型 | 木牛流马模型 | 12 | 件 | 保留旧商品故事，正常偏低 |
| 隆中对竹简礼盒 | 隆中对竹简礼盒 | 36 | 套 | 提升商品品类丰富度 |
| 新野火攻纪念帆布袋 | 新野火攻纪念帆布袋 | 18 | 个 | 文创周边扩展 |
| 孔明灯香薰套装 | 孔明灯香薰套装 | 5 | 盒 | 默认阈值 5 的临界样例 |
| 草船借箭桌游卡牌 | 草船借箭桌游卡牌 | 4 | 盒 | 新的最低库存锚点 |
...
| purchase_in / sales_out | 正数支出/收入 | 覆盖其余文创商品，库存流水总数不少于 10 条 |

验收：

- `/seller/stock` 能看到至少 6 个商品库存条目。
- `草船借箭桌游卡牌（4盒）` 是库存最低商品。
- `/seller/summary` 的 `low_stock_items` 至少包含 `草船借箭桌游卡牌` 与 `孔明灯香薰套装`。
- `/seller/summary` 能汇总销售收入、采购支出、库存金额概览。
- `/seller/chat` 回答“库存最低的商品是什么？”时指向 `草船借箭桌游卡牌（4盒）`。
...
- `/spaces/current/timeline` 至少 6 个事件，每个关键事件 payload 至少能支撑 `信息流/物流/人流` 展示。
